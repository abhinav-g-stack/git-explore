...
export function ProductFormDialog({ product, children, mode }: ProductFormDialogProps) {
  ...
  const handleGenerateImage = async () => {
    ...
    if (!name || !category) {
      toast({
        ...
        description: 'Please provide a product name and category to generate an image.',
      });
      return;
    }
    ...
    try {
      const result = await generateProductImage({ productName: name, productCategory: category });
      if (result.imageUrl) {
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            {mode === 'edit' ? 'Make changes to your product here.' : 'Fill in the details for the new product.'}
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
                ...
                <FormField control={form.control} name="imageUrl" render={({ field }) => (
                    <FormItem>
                      ...
                      <div className="flex items-center gap-2">
                        <FormControl><Input placeholder="https://... or generate one" {...field} /></FormControl>
                        <Button type="button" variant="outline" size="icon" onClick={handleGenerateImage} disabled={isGenerating || isSubmitting}>
                      ...
                      {field.value && (field.value.startsWith('data:image') || field.value.startsWith('http')) && (
                        <div className="mt-2 relative aspect-video w-full">
                            <Image src={field.value} alt="Image preview" fill className="rounded-md object-cover" />
                        </div>
                      )}
                    </FormItem>
                  )}
