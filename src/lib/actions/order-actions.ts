...
export async function getSalesPerformanceData() {
    ...
    orders.forEach(order => {
        ...
        monthlySales[month].units += order.items.reduce((sum, item) => sum + item.quantity, 0);
    });

    // Convert to array format expected by the chart
    const performanceData = Object.entries(monthlySales).map(([month, data]) => ({
        month,
        ...data,
...
export async function getCategorySalesData() {
    ...
    return Object.entries(categorySales).map(([name, value], index) => ({
        name,
        value,
        fill: categoryColors[name as keyof typeof categoryColors] || `hsl(var(--chart-${(index % 5) + 1}))`,
    }));
}
